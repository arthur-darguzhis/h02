...
import { AdminBanOrUnbanBlogCommand } from '../../../super-admin/blogs/use-cases/admin-ban-or-unban-blog.use-case';
...
describe('Return posts list related to special blog', () => {
  ...
  it(`Get posts list in a blog as a guest "extendedLikesInfo.myStatus" should be "None"`, async () => {
    const data = await queryBus.execute(
      new GetPostsListByBlogIdQuery(blog1.id, 'createdAt', 'desc', 10, 1),
    );
  ...
  it('Check order', async () => {
    const data = await queryBus.execute(
      new GetPostsListByBlogIdQuery(blog1.id, 'createdAt', 'asc', 10, 1),
    );
  ...
  it('Check that pagination works', async () => {
    const data = await queryBus.execute(
      new GetPostsListByBlogIdQuery(blog1.id, 'createdAt', 'asc', 1, 2),
    );
