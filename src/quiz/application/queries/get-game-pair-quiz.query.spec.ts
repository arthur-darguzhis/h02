...
import { GetGamePairQuizQuery } from './get-game-pair-quiz.query';
...
describe('Set publish status to quiz question', () => {
  ...
  it(`Throw error if There is no one active game for current user`, async () => {
    await expect(
      queryBus.execute(new GetGamePairQuizQuery(firstUser.id)),
    ).rejects.toThrow(
  ...
  it(`Throw error if current user is already participating in active pair`, async () => {
