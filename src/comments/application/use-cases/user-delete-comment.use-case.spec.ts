...
import { UserDeleteCommentCommand } from './user-delete-comment.use-case';
...
describe('Blogger delete blog', () => {
  ...
  it(`The user is not allowed to delete a comment that belongs to another user`, async () => {
    await expect(
      commandBus.execute(
        new UserDeleteCommentCommand(commentOfReader1.id, reader2.id),
      ),
    ).rejects.toThrow(
  ...
  it(`User delete comment`, async () => {
    await commandBus.execute(
      new UserDeleteCommentCommand(commentOfReader1.id, reader1.id),
    );
